...
#include <stdio.h>
int main() {
    int st[10], i, maxA = 0, maxS = 1;
    printf("Enter attendance of 10 students (0 to 7 days):\n\n");
    for(i = 0; i < 10; i++) {
        do {
            ...
            if(st[i] < 0 || st[i] > 7)
                printf("Invalid! Enter value between 0 and 7 only.\n");
    ...
    printf("\n");
    for(i = 0; i < 10; i++)
        printf("Student %d: %d out of 7 days\n", i + 1, st[i]);

    printf("\nHighest Attendance: Student %d (%d days)\n", maxS, maxA);
